...
const SearchStages = ({ searchInfo }: { searchInfo: SearchInfo }) => {
  ...
  return (
    <div className="mb-3 mt-1 relative pl-4">
      {/* Search Process UI */}
      <div className="flex flex-col space-y-4 text-sm text-slate-100">
        {/* Searching Stage */}
          ...
          <div className="relative">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-wrap gap-2 pl-2 mt-1">
                <div className="bg-slate-700 text-xs px-3 py-1.5 rounded border border-slate-700 inline-flex items-center">
                  <svg
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(searchInfo.urls) ? (
                      searchInfo.urls.map((url, index) => (
                        <div
                          key={index}
                          className="bg-slate-700 text-xs px-3 py-1.5 rounded border border-slate-700 truncate max-w-[200px] transition-all duration-200 hover:bg-slate-800"
                        >
                          {typeof url === "string"
                            ? url
                      ...
                      <div className="bg-slate-700 text-xs px-3 py-1.5 rounded border border-slate-700 truncate max-w-[200px] transition-all duration-200 hover:bg-slate-800">
...
const MessageArea = ({ messages }: { messages: Message[] }) => {
  ...
  useEffect(() => {
    const messageAreaContainer = messageAreaRef.current;
    if (messageAreaContainer) {
      messageAreaContainer.scrollTo({
        top: messageAreaContainer.scrollHeight,
        behavior: "smooth",
      });
    }
  }, [messages]);
            ...
            <div className="flex flex-col max-w-md">
              ...
              <div
                className={`rounded-lg py-3 px-5 ${
                  message.isUser
                    ? "bg-gradient-to-r from-purple-800 to-indigo-800 text-slate-100 rounded-br-none shadow-md"
                    : "bg-slate-700 text-slate-100 border border-slate-600 rounded-bl-none shadow-sm"
                }`}
              >
                {message.isLoading ? (
                  ...
                  <Markdown
                    ...
                    components={{
                      a: (props) => (
                        <a
                          className="text-blue-500 underline hover:text-blue-600 duration-200"
                          href={props.href}
                          target="_blank"
                          rel="noreferrer"
                        >
